...
- **Specular family (SpecularController, SpriteShapeSpecularController, SpecularLight2D, SpecularLight2DManager, ITintReceiver)** — moved to `Assets/Scripts/Core/Rendering/` (namespace `Core.Rendering`): generic rendering with no gameplay dependencies. Full docs in that folder's context.md. The gameplay subclass `World/OreSpecularController` stays here.
