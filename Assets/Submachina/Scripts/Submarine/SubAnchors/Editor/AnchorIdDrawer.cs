...
{
    /**
     * Odin value drawer that renders AnchorId fields as a searchable
     * categorized dropdown instead of a raw integer.
     *
     * Reflects over SubAnchors to discover all declared AnchorId fields,
     * groups them by category using SubAnchors.CategoryNames, and rebuilds
     * the list on domain reload. 1:1 mirror of FeedbackIdDrawer.
     */
    public class AnchorIdDrawer : OdinValueDrawer<AnchorId>
    {
        // Shared across all drawer instances — rebuilt once per domain reload.
        private static string[] _displayNames;
        private static AnchorId[] _values;
        ...
        protected override void DrawPropertyLayout(GUIContent label)
        {
            ...
            AnchorId current = this.ValueEntry.SmartValue;

            // Resolve current selection index (uses AnchorId.Equals via IEquatable).
        ...
        /**
         * Discovers every static readonly AnchorId field on SubAnchors,
         * groups by category, and builds the parallel display/value arrays.
         */
        private static void BuildEntries()
        {
            var entries = new List<(string display, AnchorId id, int category)>();

            // "(None)" sentinel for unset / default-constructed AnchorId.
            ...
            foreach (var field in typeof(SubAnchors).GetFields(BindingFlags.Public | BindingFlags.Static))
            {
                ...
                string category = SubAnchors.CategoryNames.TryGetValue(id.Category, out var catName)
                    ? catName
            ...
            _values = new AnchorId[entries.Count];
        ...
        /** Reset on domain reload so new anchor definitions are picked up. */
