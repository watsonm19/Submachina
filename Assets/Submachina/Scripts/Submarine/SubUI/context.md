...
- **SonarHud** (`Sonar/SonarHud.cs`) — compass-ring contact display. Lazily subscribes to `Sub.Sonar.ContactReturned` and spawns a fading blip (built in code — no prefab required) on a ring around the sub per echo. Presentation is **tier-gated** off `Sub.Sonar.CurrentTier`: Presence pins a generic blip to the top of the ring (no bearing); Direction places it by the contact's bearing; Size maps its distance to ring radius and shows a distance label; Identify gives it the signature's icon, color, and name. Assign `labelFont` to enable the text; blips fade over `blipFadeDuration`. The outgoing-pulse visual is intentionally left to event wiring (`Sub.Sonar.onPingEmitted` → MMF / ripple), per the compose-juice-from-events convention. See `../SubSystems/context.md` → *Sonar* for the data source.
